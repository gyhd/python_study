...
with open('D:/data/dir_label.txt', 'r', encoding='utf-8') as f:
    labels = f.readlines()
    labels = list(map(lambda x:x.strip().split('\t'), labels))
    
if __name__ == "__main__":
    test_list = 'D:/data/test.txt'
    ...
    model = model.cuda()
    # 加载训练好的模型
    checkpoint = torch.load('D:\BaiduNetdiskDownload\model_best_checkpoint_resnet50.pth.tar')
